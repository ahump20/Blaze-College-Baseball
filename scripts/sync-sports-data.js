...
if (missingDbEnvVars.length > 0) {
  console.error(
    `Missing required database environment variable(s): ${missingDbEnvVars.join(', ')}. Populate these values via your secrets sync (e.g., API_KEYS_MASTER.js → npm run mcp:sync) or export them manually before executing sync-sports-data.`
  );
  process.exit(1);
}
