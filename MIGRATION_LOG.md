...
| 2025-10-13 | Foundation | Introduced Prisma user/subscription schema to support Clerk auth, favorites, alert preferences, and Diamond Pro tiers; added Stripe endpoints for checkout/portal access. | `apps/web/prisma/schema.prisma`; new API routes under `app/api/v1/auth/*` and `app/api/v1/billing/*`; subscription context components. | This commit |
