/**
 * REAL NFL Data API Function for Cloudflare Pages
 * Handles teamId via query string parameter
 */

export async function onRequest({ request }) {
  const url = new URL(request.url);
  const teamId = url.searchParams.get('teamId') || '10'; // Tennessee Titans default
  ...
  try {
    ...
    if (scoreboardData.events) {
      scoreboardData.events.forEach(game => {
        ...
        if (competitors) {
          const targetTeam = competitors.find(c => c.id === String(teamId));
          const opponent = competitors.find(c => c.id !== String(teamId));

          if (targetTeam && opponent) {
            const teamScore = parseInt(targetTeam.score);
