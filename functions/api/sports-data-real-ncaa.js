/**
 * NCAA Data API Function for Cloudflare Pages
...
async function fetchRealNCAA(teamId) {
  ...
  try {
    ...
    if (rankingsData.rankings) {
      for (const poll of rankingsData.rankings) {
        const rank = poll.ranks?.find(r => r.team?.id === String(teamId));
        if (rank) {
          teamRanking = rank.current;
          break;
        }
      }
    }
    ...
    // Calculate strength of schedule (simplified)
    const sosRank = teamRecord.stats?.find(s => s.name === 'strengthOfSchedule')?.value || null;
    ...
    return {
      ...
      analytics: {
        ...
        strengthOfSchedule: sosRank,
        conferenceRecord: teamRecord.stats?.find(s => s.name === 'conferenceRecord')?.displayValue || 'N/A',
