...
declare function mcpCall<TInput, TOutput>(
  toolName: "get-library-docs" | "sports-context" | "inject-sports-context",
  input: TInput
): Promise<TOutput>;
...
test("docs: Recharts ResponsiveContainer props are stable and version-matched", async () => {
  ...
  const r1 = await mcpCall(
    "get-library-docs",
    { context7CompatibleLibraryID: libraryID, topic, tokens }
  );
...
test("supplement: sports context is bounded and never pollutes docs", async () => {
  ...
  const docs = await mcpCall(
    "get-library-docs",
    { context7CompatibleLibraryID: libraryID, topic, tokens: 2000 }
  );
  const supp = await mcpCall(
    "sports-context",
    { sport: "baseball", league: "MLB", team: "Cardinals", timeframe: "live", maxTokens: 300 }
  );
